  ...
  return (
    <section className="bg-white rounded-[20px] border border-[#12302E]/10 shadow-sm p-6 flex flex-col items-center text-center gap-4">
      ...
      <div className="relative w-32 h-32 flex items-center justify-center">
        <div className={`absolute inset-0 rounded-full bg-[#D8E8E4] transition-transform duration-[4000ms] ease-in-out ${running ? scale : "scale-75"}`} />
        <div className="absolute inset-4 rounded-full bg-[#0D6E64]/20" />
        <span className="relative text-sm font-medium text-[#0D6E64]">{running ? label : "Ready"}</span>
      </div>
      <p className="text-xs text-[#4A544C] max-w-xs">Four seconds in, four seconds hold, four seconds out. Repeat for as long as it helps.</p>
      <button onClick={() => setRunning((r) => !r)} className="px-5 py-2.5 rounded-full text-sm font-semibold text-[#F7F4EC] bg-[#0D6E64] shadow-sm hover:brightness-110 hover:shadow-md hover:-translate-y-0.5 active:scale-[0.97] transition-all duration-150 cursor-pointer">
      ...
      <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
        {line.numbers.map((n) => (
          <a key={n} href={telHref(n)}>
            <span className={`flex items-center gap-1.5 text-xs font-semibold text-white px-3.5 py-2.5 rounded-lg transition-colors cursor-pointer whitespace-nowrap ${styles.button}`}>
        ...
        {line.whatsapp && (
          <a href={`https://wa.me/${line.whatsapp}`} target="_blank" rel="noopener noreferrer">
            <span className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-3.5 py-2.5 rounded-lg transition-colors cursor-pointer whitespace-nowrap">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {navItems.map((item) => (
          <a key={item.id} href={`#${item.id}`} className="text-xs font-medium text-[#12302E] bg-white border border-[#12302E]/12 rounded-full px-3.5 py-2 hover:border-[#0D6E64]/40 hover:text-[#0D6E64] transition-colors cursor-pointer">
            {item.label}
          </a>
        ))}
      </div>
      ...
      <section id="emergency" className="bg-[#FAECE7] border border-[#F0997B]/40 rounded-[20px] p-5 space-y-4 scroll-mt-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-[#4A1B0C] text-sm tracking-tight">If you or someone else is in immediate danger</h2>
        </div>
      ...
      <div className="space-y-4">
        {categories.map((cat) => {
          ...
          return (
            <section key={cat.id} id={cat.id} className="bg-white rounded-[20px] border border-[#12302E]/10 shadow-sm p-5 scroll-mt-4">
