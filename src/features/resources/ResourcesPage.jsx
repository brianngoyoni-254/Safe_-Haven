...
const RESOURCES = [
  ...
  // NACADA Regional Offices for counties with no private rehab
  {
    id: 69,
  ...
  },
  {
    ...
    videos: [
      ...
      },
      {
        ...
        duration: "Habari fupi",
        desc: "Kwa Kiswahili. Ripoti kutoka Mombasa kuhusu vijana wanaotaka kuachana na uraibu wa mihadarati na upungufu wa dawa ya methadone wanaokabiliana nayo.",
...
function makePinIcon(color, isSelected) {
  ...
  return L.divIcon({
    className: "",
    html: `<div style="width:${size}px;height:${size}px;background:${color};border:2px solid white;border-radius:50% 50% 50% 0;transform:rotate(-45deg);box-shadow:0 2px 6px rgba(0,0,0,0.35);"></div>`,
    iconSize: [size, size],
...
function makeUserIcon() {
  return L.divIcon({
    className: "",
    html: `<div style="position:relative;width:18px;height:18px;"><div style="position:absolute;inset:0;border-radius:9999px;background:rgba(13,148,136,0.25);animation:sh-pulse 1.6s ease-out infinite;"></div><div style="position:absolute;inset:4px;border-radius:9999px;background:#0d9488;border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.4);"></div></div><style>@keyframes sh-pulse{0%{transform:scale(0.6);opacity:0.8;}100%{transform:scale(2.2);opacity:0;}}</style>`,
    iconSize: [18, 18],
    iconAnchor: [9, 9],
  });
}
...
export default function Resources() {
  ...
  const filtered = useMemo(() => {
    ...
    let list = withDistance.filter((r) => {
      ...
      const matchDistance = !userLocation || maxDistanceKm == null || r.distanceKm <= maxDistanceKm;
      const matchSearch = !q || r.name.toLowerCase().includes(q) || r.address.toLowerCase().includes(q) || r.county.toLowerCase().includes(q) || r.type.toLowerCase().includes(q);
      return matchType && matchRegion && matchDistance && matchSearch;
    });
    if (userLocation) list = [...list].sort((a, b) => a.distanceKm - b.distanceKm);
    return list;
  ...
  const handleUseMyLocation = () => {
    ...
    if (!window.isSecureContext) {
      setLocationError("Your browser is blocking location because this page isn't served over HTTPS. Try https:// or localhost.");
      return;
    }
    setLocating(true);
    setLocationError("");
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === err.PERMISSION_DENIED) {
          setLocationError("Location access was denied. Check your browser's site settings and allow location for this page, then try again.");
    ...
    return LIBRARY_TOPICS
      ...
      .map((topic) => ({
        ...
        readings: topic.readings.filter((r) => {
          if (!q) return true;
          return r.title.toLowerCase().includes(q) || r.publisher.toLowerCase().includes(q) || r.desc.toLowerCase().includes(q);
        }),
      }))
    ...
    return VIDEO_LIBRARY
      ...
      .map((topic) => ({
        ...
        videos: topic.videos.filter((v) => {
          if (!q) return true;
          return v.title.toLowerCase().includes(q) || v.publisher.toLowerCase().includes(q) || v.desc.toLowerCase().includes(q);
        }),
      }))
        ...
        <>
          <div className="flex gap-3 flex-wrap items-center">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-lg border border-[#12302E]/15 text-sm text-[#12302E] focus:outline-none focus:ring-2 focus:ring-[#0D6E64] focus:border-transparent transition-shadow"
              />
            </div>
            ...
            <select
              ...
              className="px-3 py-2 rounded-lg border border-[#12302E]/15 text-sm text-[#4A544C] bg-white focus:outline-none focus:ring-2 focus:ring-[#0D6E64] focus:border-transparent cursor-pointer"
            >
              {REGIONS.map((r) => (
            ...
            <button
              ...
              disabled={locating}
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-white bg-[#0D6E64] hover:brightness-110 active:scale-[0.98] transition-all duration-150 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
            >
            ...
            {userLocation && (
              <button
                ...
                className="px-3 py-2 rounded-lg text-sm font-medium text-[#4A544C] bg-[#EFEAE0] hover:bg-[#12302E]/10 transition-colors cursor-pointer"
              >
                Clear
              </button>
            )}
                    ...
                    >
                      <Popup>
                        <div className="text-sm space-y-1 min-w-[180px]">
                          ...
                          {r.distanceKm !== null && (
                            <p className="text-[#0D6E64] text-xs font-medium">{r.distanceKm.toFixed(0)} km away</p>
                          )}
                          <div className="flex gap-2 pt-1 flex-wrap">
                            {r.phone && (
                              <a href={`tel:${r.phone}`} className="text-xs font-medium text-[#0D6E64] underline">
                                Call
                              </a>
                            )}
                            <a
                ...
                <div key={r.id} ref={(el) => (listRefs.current[r.id] = el)}>
                  <button
                    ...
                    className={`w-full text-left bg-[#F7F4EC] rounded-[20px] border shadow-sm p-4 transition-colors cursor-pointer ${
                      selected === r.id
                        ? "border-[#0D6E64]/40 bg-[#D8E8E4]/40"
                        : "border-[#12302E]/10 hover:border-[#0D6E64]/30"
                    }`}
                  >
          ...
          <div className="bg-[#FCE7EF] border border-[#c2417a]/25 rounded-2xl p-4 flex items-center gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-[200px]">
              ...
              <p className="text-xs text-[#8a2340]">Free, confidential, 24/7 counseling and referrals — call 1192 via Safaricom or Telkom.</p>
            </div>
            <a href="tel:1192">
      ...
      {activeSection === "library" && (
        <div className="space-y-5">
          ...
          {filteredLibrary.map((topic) => (
            <section key={topic.id} className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5">
                ...
                {topic.readings.map((r) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h3 className="font-semibold text-[#12302E] text-sm leading-snug">{r.title}</h3>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mb-2">
                      <span className={`text-xs px-1.5 py-0.5 rounded-full border ${topic.badge}`}>{r.format}</span>
          ...
          <div className="bg-[#EFEAE0] border border-[#12302E]/10 rounded-2xl p-5 flex items-start gap-3">
            ...
            <p className="text-sm text-[#4A544C]">
              ...
              <a href="tel:1192" className="text-[#0D6E64] font-medium underline">1192</a>.
            </p>
          </div>
        </div>
      )}
                ...
                {topic.videos.map((v) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mb-2 ml-[22px]">
                      <span className={`text-xs px-1.5 py-0.5 rounded-full border ${topic.badge}`}>{v.format}</span>
