...
function StreakRing({ soberDays, streak, nextMilestone }) {
  ...
  const progress = nextMilestone ? Math.min(1, soberDays / nextMilestone.days) : 1;
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        ...
        <circle cx={size / 2} cy={size / 2} r={r} stroke="url(#ringGrad)" strokeWidth={stroke} fill="none" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" style={{ transition: "stroke-dashoffset 0.8s ease-out" }} />
        <defs>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <div className="text-[32px] leading-none font-bold text-[#12302E] tabular-nums" style={serif}>{soberDays !== null ? soberDays : "—"}</div>
...
function MoodChart({ checkIns }) {
  ...
  const data = last14.map((c) => ({ date: formatDate(c.date, { month: "short", day: "numeric" }), mood: c.mood }));
    ...
    <section className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5">
      ...
      <ResponsiveContainer width="100%" height={120}>
        <AreaChart data={data} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
          ...
          <Tooltip contentStyle={{ fontSize: 12, border: "1px solid #12302E1A", borderRadius: 10, background: "#F7F4EC" }} formatter={(v) => [v, "Mood"]} />
          <Area type="monotone" dataKey="mood" stroke="#0D6E64" strokeWidth={2} fill="url(#moodGrad)" dot={{ r: 3, fill: "#C98A3E" }} />
        </AreaChart>
      </ResponsiveContainer>
    </section>
  );
...
export default function Dashboard() {
  ...
  const secondaryStats = [
    { label: "Sober Streak", value: `${streak}d`, Icon: CheckCircle2, color: "#0d9668", bg: "#E3F5EC", sub: `${streak} consecutive sober days`, to: "/milestones" },
    { label: "Avg Mood", value: moodAvg ? `${moodAvg}/5` : "—", Icon: Heart, color: "#c2417a", bg: "#FCE7EF", sub: "Last 7 check-ins", to: "/check-in" },
    { label: "Sober Rate", value: soberPercentage !== null ? `${soberPercentage}%` : "—", Icon: TrendingUp, color: "#0D6E64", bg: "#D8E8E4", sub: "Last 30 days", to: "/milestones" },
  ];

  const quickLinks = [
    { to: "/groups", Icon: Users, label: "Support Groups", desc: "Find your community", color: "#0D6E64", bg: "#D8E8E4" },
    { to: "/journal", Icon: BookOpen, label: "Journal", desc: "Reflect on your day", color: "#7c5cbf", bg: "#EEE9FA" },
    { to: "/resources", Icon: MapPin, label: "Find Resources", desc: "Nearby services", color: "#0d9668", bg: "#E3F5EC" },
    { to: "/crisis", Icon: Phone, label: "Crisis Support", desc: "Help is here 24/7", color: "#c2417a", bg: "#FCE7EF" },
  ];

  if (loading) {
    return <div className="max-w-4xl mx-auto py-16 text-center text-[#4A544C] text-sm">Loading your dashboard…</div>;
  }

  if (error) {
    return <div className="max-w-4xl mx-auto py-16 text-center text-sm text-[#c2417a]">{error}</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        <Link to="/check-in" className="flex-1 min-w-[180px] flex items-center justify-center gap-2 py-3.5 rounded-2xl text-sm font-semibold text-[#F7F4EC] bg-[#0D6E64] shadow-sm hover:brightness-110 hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150 cursor-pointer">
          <Heart className="w-4 h-4" /> {todayCheckIn ? "Edit today's check-in" : "Check in today"}
        </Link>
        <Link to="/journal" className="flex-1 min-w-[180px] flex items-center justify-center gap-2 py-3.5 rounded-2xl text-sm font-semibold text-[#12302E] bg-[#F7F4EC] border border-[#12302E]/12 hover:border-[#0D6E64]/40 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150 cursor-pointer">
      ...
      {!user?.sobriety_start && (
        <section className="bg-[#D8E8E4] border border-[#0D6E64]/15 rounded-[20px] p-5 flex items-center justify-between gap-4 flex-wrap">
          ...
          <Link to="/profile" className="px-4 py-2.5 rounded-full text-sm font-semibold text-[#F7F4EC] bg-[#0D6E64] shadow-sm hover:brightness-110 transition-all cursor-pointer flex-shrink-0">Set date</Link>
        </section>
      )}
      ...
      <section className="bg-[#12302E] rounded-[24px] p-6 md:p-7 flex flex-col md:flex-row items-center gap-7 overflow-hidden relative">
        <div className="absolute inset-0 opacity-25 pointer-events-none" style={{ background: "radial-gradient(circle at 85% 20%, #C98A3E 0%, transparent 40%)" }} />
        <StreakRing soberDays={soberDays} streak={streak} nextMilestone={nextMilestone} />
        <div className="relative flex-1 w-full min-w-0">
          {soberDays !== null ? (
            <>
              <p className="text-sm text-[#D8E8E4] mb-1">
                Since <strong className="text-[#F7F4EC]">{formatDate(user.sobriety_start, { month: "long", day: "numeric", year: "numeric" })}</strong>
              </p>
              {nextMilestone ? (
                <>
                  <div className="flex items-center justify-between mb-2 flex-wrap gap-1.5">
                    <span className="text-sm font-semibold text-[#F7F4EC]">Next milestone: {nextMilestone.days} days</span>
                    <span className="text-xs text-[#F1DEBC] font-medium">{nextMilestone.days - soberDays} to go</span>
                  </div>
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${Math.min(100, (soberDays / nextMilestone.days) * 100)}%`, background: "linear-gradient(90deg, #0D6E64, #C98A3E)", transition: "width 0.7s ease-out" }} />
                  </div>
                </>
              ) : (
                <p className="text-sm text-[#F1DEBC] font-medium">Every milestone earned. Extraordinary — truly.</p>
              )}
            </>
          ) : (
            <>
              <p className="text-sm font-semibold text-[#F7F4EC] mb-1">No start date set yet</p>
              <p className="text-sm text-[#D8E8E4]">Add your recovery start date to begin tracking days and unlocking milestone badges.</p>
            </>
          )}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {secondaryStats.map((stat) => (
          <Link key={stat.label} to={stat.to} className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm hover:shadow-md hover:border-[#0D6E64]/30 hover:-translate-y-0.5 transition-all duration-150 cursor-pointer p-5 flex items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="text-[22px] leading-none font-bold text-[#12302E] tabular-nums tracking-tight">{stat.value}</div>
        ...
        <section className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5">
          ...
          <div className="flex items-center gap-3.5 flex-wrap">
            ...
            <div className="flex-1 min-w-[160px]">
              ...
              <p className="flex items-center gap-1.5 text-xs text-[#4A544C] mt-0.5"><Clock className="w-3 h-3" /> {upcomingSession.time}</p>
            </div>
            <Link to="/groups" className="px-4 py-2.5 rounded-full text-sm font-semibold text-[#F7F4EC] bg-[#0D6E64] shadow-sm hover:brightness-110 transition-all cursor-pointer flex-shrink-0">Open chat</Link>
          </div>
        </section>
      )}
        ...
        <section className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5">
          ...
          <div className="flex flex-wrap gap-3">
            {earnedMilestones.map((m) => (
              <div key={m.days} className="flex items-center gap-2 rounded-xl px-3.5 py-2" style={{ background: m.bg }}>
          ...
          <div className="flex items-center gap-5 flex-wrap">
            <div className="flex items-center gap-1.5 text-sm">
              {todayCheckIn.soberToday ? (
                <><CheckCircle2 className="w-4 h-4 text-[#0d9668]" /><span className="text-[#0d9668] font-semibold">Sober today</span></>
              ) : (
                <><XCircle className="w-4 h-4 text-[#c2417a]" /><span className="text-[#c2417a] font-semibold">Not sober today</span></>
              )}
            </div>
            <span className="text-sm text-[#4A544C]">Mood: <strong className="text-[#12302E] font-semibold">{todayCheckIn.mood}/5</strong></span>
            <span className="text-sm text-[#4A544C]">Cravings: <strong className="text-[#12302E] font-semibold">{todayCheckIn.cravingLevel}/5</strong></span>
            {todayCheckIn.notes && <p className="text-xs text-[#4A544C] italic w-full">{todayCheckIn.notes}</p>}
          </div>
        ) : (
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <p className="text-sm text-[#4A544C]">You haven't checked in yet today. How are you feeling?</p>
            <Link to="/check-in" className="px-4 py-2.5 rounded-full text-sm font-semibold text-[#F7F4EC] bg-[#0D6E64] shadow-sm hover:brightness-110 transition-all cursor-pointer flex-shrink-0">Check in now</Link>
          </div>
        )}
      </section>
      ...
      <section className="bg-[#12302E] rounded-[20px] p-6 text-center relative overflow-hidden">
        ...
        <p className="italic text-[15px] text-[#F1DEBC] leading-relaxed max-w-md mx-auto" style={serif}>"{quote}"</p>
      </section>
      ...
      <section className="bg-[#F1DEBC]/45 border border-[#C98A3E]/25 rounded-[20px] p-5 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-[#4A544C] mt-0.5">A small M-Pesa donation helps keep it free for everyone in recovery.</p>
          </div>
        </div>
        <Link to="/donations" className="px-4 py-2.5 rounded-full text-sm font-semibold text-[#12302E] bg-[#C98A3E] shadow-sm hover:brightness-105 hover:shadow-md hover:-translate-y-0.5 active:scale-[0.97] transition-all duration-150 cursor-pointer flex-shrink-0">Support us</Link>
      </section>
        ...
        {quickLinks.map((link) => (
          <Link key={link.to} to={link.to} className="cursor-pointer group">
            <div className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm hover:shadow-md hover:border-[#0D6E64]/30 hover:-translate-y-0.5 transition-all duration-150 h-full p-4 flex items-center gap-3.5">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: link.bg }}>
                <link.Icon className="w-[18px] h-[18px]" style={{ color: link.color }} />
              </div>
              <div className="min-w-0">
                <div className="text-sm font-semibold text-[#12302E] leading-tight tracking-tight">{link.label}</div>
