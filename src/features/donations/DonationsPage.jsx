...
const IMPACT_ITEMS = [
  { Icon: LifeBuoy, title: "Keeps crisis support free", desc: "Every hotline and breathing exercise on the Crisis page stays free and ad-free for anyone who needs it." },
  { Icon: Users, title: "Grows peer support groups", desc: "Donations fund the infrastructure that keeps group chats and check-ins running smoothly as our community grows." },
  { Icon: BookOpen, title: "Expands the resource library", desc: "Helps us keep adding vetted treatment centers, counselors, and recovery literature for Kenyans in recovery." },
];
  ...
  return (
    <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {IMPACT_ITEMS.map((item) => (
        <div key={item.title} className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5">
...
function AmountForm({ amount, setAmount, customAmount, setCustomAmount, frequency, setFrequency, name, setName, message, setMessage, anonymous, setAnonymous }) {
  return (
    <section className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5 space-y-5">
      ...
      <div className="inline-flex rounded-full bg-[#EFEAE0] p-1 text-sm font-medium">
        {["once", "monthly"].map((f) => (
          <button key={f} type="button" onClick={() => setFrequency(f)} className={`px-4 py-2 rounded-full transition-all cursor-pointer ${frequency === f ? "bg-[#0D6E64] text-[#F7F4EC] shadow-sm" : "text-[#4A544C] hover:text-[#12302E]"}`}>
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {PRESET_AMOUNTS.map((v) => (
          <button key={v} type="button" aria-pressed={amount === v && !customAmount} onClick={() => { setAmount(v); setCustomAmount(""); }} className={`py-3 rounded-xl text-sm font-semibold border-2 transition-all cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0D6E64] focus-visible:ring-offset-2 ${amount === v && !customAmount ? "border-[#0D6E64] bg-[#D8E8E4] text-[#0D6E64]" : "border-[#12302E]/10 text-[#12302E] hover:border-[#0D6E64]/40"}`}>
            {formatKES(v)}
          </button>
        ))}
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-[#4A544C]/70 font-medium">KES</span>
          <input type="number" min="1" placeholder="1,000" value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setAmount(null); }} className="w-full rounded-xl border border-[#12302E]/15 pl-12 pr-3 py-2.5 text-sm text-[#12302E] bg-white placeholder-[#4A544C]/40 focus:outline-none focus:ring-2 focus:ring-[#0D6E64] focus:border-transparent transition-shadow" />
        </div>
      </div>
      ...
      <div className="space-y-4">
        <label className="flex items-center gap-2.5 text-sm text-[#12302E] select-none cursor-pointer">
          <input type="checkbox" checked={anonymous} onChange={(e) => setAnonymous(e.target.checked)} className="w-4 h-4 rounded border-[#12302E]/25 text-[#0D6E64] focus:ring-[#0D6E64] cursor-pointer" />
          Give anonymously
        </label>
        {!anonymous && (
          <div>
            <label className="block text-sm font-medium text-[#12302E] mb-1.5">Your name (optional)</label>
            <input type="text" placeholder="e.g. Phoenix, or your real name" value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-[#12302E]/15 px-3 py-2.5 text-sm text-[#12302E] bg-white placeholder-[#4A544C]/40 focus:outline-none focus:ring-2 focus:ring-[#0D6E64] focus:border-transparent transition-shadow" />
          </div>
        )}
        <div>
          <label className="block text-sm font-medium text-[#12302E] mb-1.5">A note of appreciation (optional)</label>
          <textarea placeholder="Say a few words — what Safe Haven has meant to you, or why you're giving." value={message} onChange={(e) => setMessage(e.target.value)} rows={3} className="w-full resize-none rounded-xl border border-[#12302E]/15 p-3 text-sm text-[#12302E] bg-white placeholder-[#4A544C]/40 focus:outline-none focus:ring-2 focus:ring-[#0D6E64] focus:border-transparent transition-shadow" />
        </div>
      </div>
    </section>
  );
...
function MpesaCard({ finalAmount, phone, setPhone, status, error, onPay, copied, onCopyTill, receiptUrl }) {
  ...
  return (
    <section className="rounded-[20px] shadow-sm overflow-hidden border" style={{ borderColor: `${MPESA_GREEN}30` }}>
      <div className="px-5 py-4 flex items-center justify-between" style={{ background: MPESA_DARK }}>
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center font-bold text-sm" style={{ background: MPESA_GREEN, color: MPESA_DARK }}>M</div>
          <div>
      ...
      <div className="bg-[#F7F4EC] p-5 space-y-4">
        {status === "success" ? (
          <div className="text-center py-6">
            <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: `${MPESA_GREEN}20` }}>
          ...
          <>
            <div>
              ...
              <input type="tel" placeholder="07XX XXX XXX" value={phone} onChange={(e) => setPhone(e.target.value)} disabled={status === "sending" || status === "pending"} className="w-full rounded-xl border border-[#12302E]/15 px-3 py-2.5 text-sm text-[#12302E] bg-white placeholder-[#4A544C]/40 focus:outline-none focus:ring-2 focus-visible:ring-offset-0 transition-shadow disabled:opacity-60" style={{ "--tw-ring-color": MPESA_GREEN }} onFocus={(e) => (e.target.style.boxShadow = `0 0 0 2px ${MPESA_GREEN}`)} onBlur={(e) => (e.target.style.boxShadow = "none")} />
              {phone && !isValidKenyanPhone(phone) && <p className="text-xs text-[#8a2340] mt-1.5">Enter a valid Safaricom number, e.g. 0722 123 456</p>}
            </div>
            ...
            <div className="flex items-center justify-between text-sm bg-white rounded-xl border border-[#12302E]/10 px-3.5 py-2.5">
              ...
              <span className="font-semibold text-[#12302E] tabular-nums">{finalAmount ? formatKES(finalAmount) : "—"}</span>
            </div>
            ...
            <button onClick={onPay} disabled={disabled} className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-semibold text-white hover:brightness-105 hover:shadow-lg active:scale-[0.99] transition-all duration-150 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: status === "sending" || status === "pending" ? MPESA_DARK : MPESA_GREEN, color: MPESA_DARK === "#1A2E1A" && (status === "sending" || status === "pending") ? "#fff" : MPESA_DARK }}>
              {status === "sending" ? <><Loader2 size={18} className="animate-spin" /> Sending prompt…</> : status === "pending" ? <><Loader2 size={18} className="animate-spin" /> Waiting for confirmation…</> : <>Send M-Pesa prompt <ArrowRight size={18} /></>}
            </button>
      ...
      {status !== "success" && (
        <div className="bg-[#F7F4EC] border-t border-[#12302E]/10 p-5">
          <div className="flex items-start gap-2 mb-3">
            ...
            <p className="text-xs text-[#4A544C]">Prefer to pay manually? Go to M-Pesa → Lipa na M-Pesa → Paybill on your phone.</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <button onClick={onCopyTill} className="bg-white rounded-xl border border-[#12302E]/10 px-3.5 py-2.5 text-left hover:border-[#0D6E64]/40 transition-colors cursor-pointer">
