...
function LoginView({ onSwitch }) {
  ...
  return (
    <Card>
      ...
      <SocialButton onClick={handleGoogle} loading={gLoading} logo={<GoogleLogo />} label="Continue with Google" />
      <Divider />
      <div className="space-y-3">
        <Input icon={Mail} type="email" placeholder="name@email.com" value={email} onChange={(e) => setEmail(e.target.value)} />
        <Input
          icon={Lock}
          ...
          rightElement={
            <button onClick={() => setShowPassword((v) => !v)} className="text-[#4A544C]/50 hover:text-[#12302E] transition-colors" tabIndex={-1}>
      ...
      <p className="text-center text-xs text-[#4A544C] mt-5">
        No account?{" "}
        <button onClick={() => onSwitch("register")} className="text-[#0D6E64] hover:text-[#12302E] hover:underline font-semibold transition-colors cursor-pointer">
          Create one
        </button>
      </p>
    </Card>
...
function RegisterView({ onSwitch }) {
  ...
  return (
    <Card>
      ...
      <Divider />
      <div className="space-y-3">
        <Input icon={User} placeholder="Anonymous display name" value={username} onChange={(e) => setUsername(e.target.value)} />
        <Input icon={Mail} type="email" placeholder="name@email.com" value={email} onChange={(e) => setEmail(e.target.value)} />
        <Input
          icon={Lock}
          type={showPassword ? "text" : "password"}
      ...
      <p className="text-center text-xs text-[#4A544C] mt-4">
        ...
        <button onClick={() => onSwitch("login")} className="text-[#0D6E64] hover:text-[#12302E] hover:underline font-semibold transition-colors cursor-pointer">
          Sign in
        </button>
      </p>
    </Card>
...
function ForgotView({ onSwitch }) {
  ...
  return (
    <Card>
      <button onClick={() => onSwitch("login")} className="flex items-center gap-1.5 text-xs text-[#4A544C] hover:text-[#12302E] mb-6 -ml-1 transition-colors cursor-pointer">
      ...
      {sent ? (
        <div className="text-center py-2">
          ...
          <button onClick={() => onSwitch("login")} className="mt-6 text-xs text-[#0D6E64] hover:text-[#12302E] hover:underline transition-colors cursor-pointer">
            Back to sign in
          </button>
        </div>
      ) : (
        <div className="space-y-3">
          <Input icon={Mail} type="email" placeholder="name@email.com" value={email} onChange={(e) => setEmail(e.target.value)} />
          <ErrorBanner message={error} />
          <PrimaryButton onClick={handleReset} loading={loading}>Send reset link</PrimaryButton>
        </div>
      )}
    </Card>
  );
  ...
  return (
    <div className="relative hidden lg:flex flex-col justify-between w-[42%] bg-[#12302E] p-12 overflow-hidden">
      ...
      <div className="relative flex items-center gap-2.5">
        ...
        <span className="font-semibold text-[#F7F4EC] text-lg tracking-tight" style={serif}>Safe Haven</span>
      </div>
      <div className="relative">
        <h2 className="text-[32px] font-medium text-[#F7F4EC] leading-[1.2] mb-6" style={serif}>
  ...
  return (
    <div className="min-h-screen flex bg-[#F7F4EC]">
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center px-6 py-16">
        <button onClick={() => navigate("/")} className="absolute top-6 left-6 flex items-center gap-1.5 text-sm font-medium text-[#4A544C] hover:text-[#12302E] transition-colors cursor-pointer">
