...
export default function Profile() {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div>
        <h1 className="text-[28px] leading-tight font-medium text-[#12302E] tracking-tight" style={serif}>My Profile</h1>
        <p className="flex items-center gap-1.5 text-sm text-[#4A544C] mt-1.5"><ShieldCheck size={14} className="text-[#0D6E64]/60" />Your profile is anonymous. Only you can see your real identity.</p>
      </div>

      <section className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5 flex items-center gap-4">
        <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-semibold text-[#F7F4EC] shadow-sm flex-shrink-0" style={{ ...serif, backgroundColor: colorFromName(user?.username) }}>{(user?.username ?? "?")[0]?.toUpperCase()}</div>
        <div className="min-w-0"><div className="font-semibold text-[#12302E] truncate tracking-tight">{user?.username ?? "Anonymous"}</div><div className="text-sm text-[#4A544C] truncate">{user?.email ?? "—"}</div></div>
      </section>
      ...
      <section className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5 space-y-4">
        ...
        <button onClick={handleSave} disabled={isSaving} className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-semibold text-white bg-[#0D6E64] hover:brightness-110 hover:shadow-lg active:scale-[0.99] transition-all duration-150 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed">
          {isSaving ? <><Loader2 size={18} className="animate-spin" /> Saving…</> : <><Save className="w-4 h-4" /> Save Profile</>}
        </button>
      </section>
      ...
      <section className="bg-[#F7F4EC] rounded-[20px] border border-[#12302E]/10 shadow-sm p-5">
        ...
        <button onClick={handleLogout} disabled={isLoggingOut} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium text-[#12302E] bg-[#EFEAE0] hover:bg-[#12302E]/10 transition-colors cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed">
          {isLoggingOut ? <><Loader2 size={16} className="animate-spin" /> Signing out…</> : <><LogOut className="w-4 h-4" /> Sign Out</>}
        </button>
      </section>
    </div>
  );
